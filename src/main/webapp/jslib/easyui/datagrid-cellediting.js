(function($){
	...
	function getCurrCell(target){
		var cell = $(target).datagrid('getPanel').find('td.datagrid-cell-selected');
		if (cell.length){
			return {
	...
	function unselectCell(target, p){
		...
		cell.removeClass('datagrid-cell-selected');
		opts.onUnselectCell.call(target, p.index, p.field);
	}

	function unselectAllCells(target){
		var panel = $(target).datagrid('getPanel');
		panel.find('td.datagrid-cell-selected').removeClass('datagrid-cell-selected');
	}

	function unselectAllRows(target){
		var panel = $(target).datagrid('getPanel');
		panel.find('tr.datagrid-row-selected').removeClass('datagrid-row-selected');
	}
	
	function uncheckAllRows(target){
		var panel = $(target).datagrid('getPanel');
		panel.find('tr.datagrid-row-checked').removeClass('datagrid-row-checked');
		panel.find('div.datagrid-cell-check').find('input[type="checkbox"]').removeAttr("checked"); 
	...
	function selectCell(target, p){
		...
		var row = opts.finder.getTr(target, p.index);
		if (!row.hasClass('datagrid-row-selected')) {
			$(target).datagrid('selectRow', p.index);
		}
		
		var cell = opts.finder.getTr(target, p.index).find('td[field="'+p.field+'"]');
		cell.addClass('datagrid-cell-selected');
	...
	function getSelectedCells(target){
		var cells = [];
		var panel = $(target).datagrid('getPanel');
		panel.find('td.datagrid-cell-selected').each(function(){
			var td = $(this);
			cells.push({
	...
	function gotoCell(target, p){
		...
		var cell = panel.find('td.datagrid-cell-selected');
		if (!cell){return;}
			...
			if (tr.length && tr.parent().length){
				...
				if (td.length){
					...
					if (opts.singleSelect){
						...
						if (opts.ctrlSelect){
							if (e.ctrlKey){
								if (td.hasClass('datagrid-cell-selected')){
	...
	function disableCellSelecting(target){
		...
		panel.unbind('.cellediting').find('td.datagrid-cell-selected').removeClass('datagrid-cell-selected');
		var dc = state.dc;
